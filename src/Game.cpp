...
bool
Game::canMakeMove(GamePosition position) noexcept {
...
bool
Game::playerHasBlocks(Player player,
...
GameReturn
Game::makeMove(const Player player, const GamePosition gamePosition) noexcept {
...
Block &
Game::getBlock(const GamePosition position) noexcept {
    // auto& a = gamePositions.at(position);
    // a.setOwnership(Player::PLAYER_2);
    // return a;
    return gamePositions.at(position);
}

std::vector<Block>
Game::getPlayerMoves(const Player player) const noexcept {
...
std::tuple<bool, Player>
Game::isGameOver() const noexcept {
    return std::make_tuple(gameOver, winner);
}

void
Game::printGame() noexcept {
    std::cout << "Print Game not implemented." << std::endl;
}